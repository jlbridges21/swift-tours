...
type SceneStripProps = {
  ...
  /** When true, skip the outer gradient wrapper (parent provides it). */
  embedded?: boolean;
  /**
   * When false (default), hide the strip if there is at most one scene —
   * matching pre-groups behavior. Set true when filtering by group so a
   * single-scene group still shows its thumb.
   */
...
export function SceneStrip({
  ...
  if (!showWhenSingle && scenes.length <= 1) {
    return null;
  }
